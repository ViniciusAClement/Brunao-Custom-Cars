...
const LoginPage = ({ setCurrentPage, loginEmail, setLoginEmail, loginPassword, setLoginPassword, loginError, handleLogin }) => (
  <div className="auth-page">
    <div className="auth-card">
      <h2>ENTRAR</h2>
      {loginError && <p className="error-message">{loginError}</p>}
      <form onSubmit={(e) => { e.preventDefault(); handleLogin(loginEmail, loginPassword); }}>
        <label>Email</label>
        <input 
          type="email" 
          value={loginEmail}
      ...
      <p className="login-hint">Teste com: <br/><strong>gerente@brunao.com</strong> / <strong>senha123</strong></p>
...
function App() {
  ...
  const handleLogin = (email, password) => {
    ...
    let role = null
    if (email === 'gerente@brunao.com' && password === 'senha123') {
      role = 'admin'
    } else if (email.toLowerCase().includes('cliente') && password === 'senha123') {
      role = 'client'
    } else {
    ...
    setLoginEmail('')
    setLoginPassword('')
    setCurrentPage(role === 'admin' ? 'admin-dashboard' : 'client-dashboard')
  }
  ...
  if (!isLoggedIn) {
    if (currentPage === 'login') return <LoginPage setCurrentPage={setCurrentPage} loginEmail={loginEmail} setLoginEmail={setLoginEmail} loginPassword={loginPassword} setLoginPassword={setLoginPassword} loginError={loginError} handleLogin={handleLogin} />
